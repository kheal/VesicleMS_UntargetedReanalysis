...
is.dat.file.1 <- "RP_org/CyanoDCM_ISvesicles.csv"
is.dat.file.2 <- NA
xcms.dat.pos.file <- "RP_org/Area_2_20208131211.txt"
...
#Import sample key----
samp.key <- read_csv(samp.key.file) %>% filter (Fraction == "CyanoDCM")
...
blank.dat <- xcms.dat_pos %>%
  filter(str_detect(SampID, "Blk"))

std.dat <- xcms.dat_pos %>%
